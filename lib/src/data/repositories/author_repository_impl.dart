...
class AuthorRepositoryImpl implements IAuthorRepository {
  ...
  AuthorRepositoryImpl({required DatabaseService databaseService})
    : _databaseService = databaseService;

  final logger = DevLogger('AuthorRepositoryImpl');

  @override
  Future<Either<Failure, List<Author>>> getAuthors() async {
    logger.info('AuthorRepositoryImpl: Entering getAuthors');
    try {
      final result = await _databaseService.getAll('authors');
      return result.fold(
  ...
  Future<Either<Failure, Author?>> getAuthorByName({
    ...
    try {
      final result = await _databaseService.query('authors', {'name': name});
      return result.fold(
        (failure) => Either.left(failure),
        (records) {
          if (records.isEmpty) {
  ...
  Future<Either<Failure, List<Author>>> getAuthorsByNames({
    ...
    try {
      ...
      final result = await _databaseService.query('authors', {'name': {'\$in': names}});
      return result.fold(
        (failure) => Either.left(failure),
        (records) {
          final authors = <Author>[];
  ...
  Future<Either<Failure, Unit>> addAuthor({required Author author}) async {
    ...
    try {
      final result = await _databaseService.transaction(() async {
        final model = AuthorModel.fromEntity(author);
        final saveResult = await _databaseService.save('authors', author.key, model.toMap());
        if (saveResult.isLeft()) {
          throw Exception(saveResult.getLeft().getOrElse(() => DatabaseFailure('Save failed')));
        }
        ...
        if (updateResult.isLeft()) {
          throw Exception(updateResult.getLeft().getOrElse(() => DatabaseFailure('Update relationships failed')));
        }
      });
      return result.fold(
        (failure) => Either.left(failure),
        (_) {
          logger.info('AuthorRepositoryImpl: Success added author ${author.name}');
    ...
    try {
      final result = await _databaseService.transaction(() async {
        ...
        if (existingEither.isLeft()) {
          throw Exception(existingEither.getLeft().getOrElse(() => DatabaseFailure('Failed to get existing author')));
        }
        ...
        if (existing != null) {
          ...
          if (removeResult.isLeft()) {
            throw Exception(removeResult.getLeft().getOrElse(() => DatabaseFailure('Remove relationships failed')));
          }
          // If key changed, delete the old record
          if (existing.key != author.key) {
            final deleteResult = await _databaseService.delete('authors', existing.key);
            if (deleteResult.isLeft()) {
              throw Exception(deleteResult.getLeft().getOrElse(() => DatabaseFailure('Delete failed')));
            }
          }
        }
        final model = AuthorModel.fromEntity(author);
        ...
        if (addResult.isLeft()) {
          throw Exception(addResult.getLeft().getOrElse(() => DatabaseFailure('Add relationships failed')));
        }
      });
      return result.fold(
        (failure) => Either.left(failure),
  ...
  Future<Either<Failure, Unit>> deleteAuthor({required Author author}) async {
    ...
    try {
      final result = await _databaseService.transaction(() async {
        final queryResult = await _databaseService.query('authors', {'name': author.name});
        if (queryResult.isLeft()) {
          throw Exception(queryResult.getLeft().getOrElse(() => DatabaseFailure('Query failed')));
        }
        ...
        if (records.isNotEmpty) {
          ...
          final deleteResult = await _databaseService.delete('authors', author.name);
          if (deleteResult.isLeft()) {
            throw Exception(deleteResult.getLeft().getOrElse(() => DatabaseFailure('Delete failed')));
          }
        }
  ...
  Future<Either<Failure, Unit>> _updateRelationshipsForAuthor(
    ...
    try {
      final booksResult = await _databaseService.getAll('books');
          ...
          for (final bookMap in bookMaps) {
            try {
              ...
              if (bookModel.authorIds.contains(authorName)) {
                ...
                final saveResult = await _databaseService.save('books', bookModel.id!, updatedBookModel.toMap());
                if (saveResult.isLeft()) {
                  return Either.left(saveResult.getLeft().getOrElse(() => DatabaseFailure('Save failed')));
                }
              }
            } catch (e) {
              return Either.left(DataParsingFailure(e.toString()));
